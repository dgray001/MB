...
public class MC_range {
	...
	public static void main(String[] args) {
        ...
        if (osname.startsWith("windows")) {
            filePath = "C:\\Users\\Yo-Dan\\Documents\\School\\Internships\\Summer 2018-Duquesne\\Java_MC_Data\\";
        } else if (osname.startsWith("linux")) {
        	filePath = "/root/Documents/MC_Data/";
        }
		...
		double pressure = 0.0;
		int PASSES = 0; // passes per run
		double kBT_i = 0.0; // inital temperature
		int kBT_s = 0; // temperature steps
		double dkBT = 0.0; // amount kBT incremented by each time
		int restart = 0; // start with ice structure
		String filename = ""; // name of restart file
		int eqPass1 = 0; // energy/density
		int eqPass2 = 0; // everything else
        
		FileReader parameters = null;
		try {
			parameters = new FileReader(filePath + "parameters");
			BufferedReader param = new BufferedReader(parameters);
			String line = param.readLine();
			...
			line = param.readLine();
			ln = line.split(" ");
			pressure = Double.parseDouble(ln[0]);
			kBT_i = Double.parseDouble(ln[1]);
			kBT_s = Integer.parseInt(ln[2]);
			dkBT = Double.parseDouble(ln[3]);
			line = param.readLine();
			ln = line.split(" ");
			PASSES = Integer.parseInt(ln[0]);
			eqPass1 = Integer.parseInt(ln[1]);
			eqPass2 = Integer.parseInt(ln[2]);
			line = param.readLine();
			ln = line.split(" ");
			restart = Integer.parseInt(ln[0]);
			if (restart > 0) {
				filename = ln[1];
			}
			param.close();
		...
		long start_time2 = start_time;
		
		// Parameters
		double kBT_c = kBT_i; // current temperature
		...
		final int printFreq = 500000*N; // how often status printed to screen
		...
		PrintWriter DATA = null; // open up overall data file
		try {
			DATA = new PrintWriter(filePath + "MC_P=" + pressure + ".txt", "UTF-8");
		}
		catch (FileNotFoundException | UnsupportedEncodingException e) {
			e.printStackTrace();
		}
		
		System.out.println("Beginning " + kBT_s + " Simulations (P* = " + pressure + ")");
		
		// Time for big fat loop
		while (currentTStep < kBT_s) {
			currentTStep ++;
			
			System.out.println("\n\n\nBeginning simulation " + currentTStep + " with kBT = " + kBT_c);
			...
			while (currentStep <= STEPS) {
				if (currentStep%volumeFreq == 0) { // volume move
					...
					double weight = (Uf-Ui) + pressure*(q[0]*q[1]-vOld) - kBT_c*N*Math.log(q[0]*q[1]/vOld); // dH = dU + PdV - kTNln(V'/V)
					if (MC_utils.Accept(weight, kBT_c)) {
						Ui = Uf;
						qAccept ++;
					}
					else {
					...
					if (parameter == 2) { // orientation move
						...
						if (MC_utils.Accept(U_f-U_i, kBT_c)) {
							Ui += U_f - U_i;
							oAccept ++;
						}
						else { // move rejected
				...
				if (currentStep%dataFreq == 0) { // Save data to lists
					...
					if (currentStep > eqPass2*N) {
						...
						double h = MC_utils.C_p(energy_list, N, kBT_c);
						double i = MC_utils.kap(density_list, N, kBT_c); // equilibrate thermodynamics
						double e = MC_utils.alpha(energy_list, density_list, N, kBT_c);
						heat_list.add(h);
...
//					try {
//						traj = new PrintWriter(filePath + "MC_traj" + Double.toString(kBT_c) + ".txt", "UTF-8");
//					}
//					catch (FileNotFoundException | UnsupportedEncodingException e) {
//						e.printStackTrace();
//					}
...
//					traj.close();
			...
			if (radial_list.size() > 0) {
				...
				try {
					radial = new PrintWriter(filePath + "MC_radial" + Double.toString(kBT_c) + ".txt", "UTF-8");
				}
				catch (FileNotFoundException | UnsupportedEncodingException e) {
					e.printStackTrace();
				}
			...
			System.out.println(MC_utils.time_message((System.currentTimeMillis() - start_time2)));
			System.out.println("Total " + MC_utils.time_message((System.currentTimeMillis() - start_time)));
			System.out.print("-----------------------------------------------\n");
			
			// Save coordinates to file
			PrintWriter pic = null;
			try {
				pic = new PrintWriter(filePath + "MC_pic" + Double.toString(kBT_c) + ".txt", "UTF-8");
			}
			catch (FileNotFoundException | UnsupportedEncodingException e) {
				e.printStackTrace();
			}
...
//			data.close();
			...
			DATA.println("T* = " + kBT_c); // write to main file
			DATA.println(MC_utils.average(energy_list) + " " + MC_utils.average(density_list) + " " + MC_utils.average(heat_list) + " " + MC_utils.average(isothermal_list) + " " + MC_utils.average(expansion_list) + " " + sdf);
			DATA.println("");
			DATA.flush();
			
			start_time2 = System.currentTimeMillis();
			currentStep = 1; // clear parameters and lists
			energy_list.clear();
			density_list.clear();
			heat_list.clear();
			isothermal_list.clear();
			expansion_list.clear();
			radial_list.clear();
			msd_list.clear();
			
			kBT_c += dkBT;
			kBT_c = Math.round(kBT_c*1000)/1000.0;
		} // big fat loop
		System.out.println("\n\n\n----------Simulations Complete----------");
		System.out.println("P*: " + pressure);
		System.out.println("T* initial: " + kBT_i);
		System.out.println("Steps: " + kBT_s + " (dT*: " + dkBT + ")");
		System.out.println("Passes/step: " + PASSES);
		System.out.println("Total " + MC_utils.time_message((System.currentTimeMillis() - start_time)));
		DATA.close();
	} // main method
} // class MC_range
